...
namespace Neo.EasyAccounts.Web.UI.Areas.Masters.Controllers
{
	...
	public class LineOfBusinessesController : Controller
	{
		...
		private readonly ILineOfBusinessService service;
		
		public LineOfBusinessesController(ILineOfBusinessService service)
		{
		...
		{
			var list = new List<LineOfBusiness>().AsEnumerable();
			try
			{
		...
		{
			var viewModel = new LineOfBusinessViewModel();
			try
			{
				if (id != null && id.HasValue)
				{
					...
					viewModel = Mapper.Map<LineOfBusinessViewModel>(entity);
		...
		public ActionResult RegEdit([Bind(Include = "ID,Name,Code,ManagerName,Description,IsActive,CreatedBy,DateCreated")] LineOfBusinessViewModel viewModel)
		{
			try
			{
				if (ModelState.IsValid)
				{
					...
					var entity = Mapper.Map<LineOfBusiness>(viewModel);
